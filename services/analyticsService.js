...
async function worksByCategory(query) {
  return { data: await Content.aggregate([{ $match: contentMatch(query) }, { $group: { _id: '$category', count: { $sum: 1 } } }, { $lookup: { from: 'categories', localField: '_id', foreignField: '_id', as: 'category' } }, { $project: { _id: 0, name: { $ifNull: [{ $first: '$category.name' }, 'à¹„à¸¡à¹ˆà¸£à¸°à¸šà¸¸à¸«à¸¡à¸§à¸”à¸«à¸¡à¸¹à¹ˆ'] }, count: 1 } }, { $sort: { count: -1 } }]) };
}

async function worksByType(query) {
  return { data: await Content.aggregate([{ $match: contentMatch(query) }, { $project: { name: { $cond: [{ $and: [{ $ne: ['$pdfFilename', ''] }, '$isPublicDownload'] }, 'à¸”à¸²à¸§à¸™à¹Œà¹‚à¸«à¸¥à¸”à¹„à¸”à¹‰', 'à¸­à¹ˆà¸²à¸™à¸£à¸²à¸¢à¸¥à¸°à¹€à¸­à¸µà¸¢à¸”'] } } }, { $group: { _id: '$name', count: { $sum: 1 } } }, { $project: { _id: 0, name: '$_id', count: 1 } }, { $sort: { count: -1 } }]) };
}
...
async function visitorTrends(query) {
  ...
  const [visitors, pageViews, searches, workViews] = await Promise.all([
    Analytics.aggregate([
      ...
      { $project: { _id: 0, date: 'const mongoose = require('mongoose');
const Analytics = require('../models/Analytics');
const User = require('../models/User');
const Content = require('../models/Content');
const Department = require('../models/Department');
...
async function topPages(query) {
  ...
  return Analytics.aggregate([
    ...
    { $sort: { views: -1 } },
    { $limit: limit },
    { $project: { _id: 0, page: 'const mongoose = require('mongoose');
const Analytics = require('../models/Analytics');
const User = require('../models/User');
const Content = require('../models/Content');
const Department = require('../models/Department');
...
async function deviceAnalytics(query) {
  ...
  const results = await Analytics.aggregate([
    ...
    { $sort: { count: -1 } },
    { $project: { _id: 0, device: 'const mongoose = require('mongoose');
const Analytics = require('../models/Analytics');
const User = require('../models/User');
const Content = require('../models/Content');
const Department = require('../models/Department');
